...
# Resolution order:
#   1. NEORUNTIME_SDK_VERSION=<version>  explicit pin (CI canaries, local overrides)
#   2. NEORUNTIME_SDK_VERSION=latest     newest release on PyPI
...
case "${NEORUNTIME_SDK_VERSION:-}" in
    latest)
        VERSION="$(pypi_latest)"
        if [ -z "$VERSION" ]; then
            echo "Error: NEORUNTIME_SDK_VERSION=latest but PyPI is unreachable" >&2
            exit 1
        fi
        echo "$VERSION"
        exit 0
        ;;
    ?*) echo "$NEORUNTIME_SDK_VERSION"; exit 0 ;;
esac

if [ ! -f "$LOCK_FILE" ]; then
    echo "Error: $LOCK_FILE not found — it pins the SDK for reproducible builds." >&2
    echo "Restore it from git, or set NEORUNTIME_SDK_VERSION to build anyway." >&2
    exit 1
fi
